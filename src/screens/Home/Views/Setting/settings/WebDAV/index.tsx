  ...
  const run = async(type: Exclude<Action, null>) => {
    ...
    if (type == 'restore') {
      const confirmed = await confirmDialog({
        message: t('setting_webdav_restore_confirm'),
        cancelButtonText: t('dialog_cancel'),
        confirmButtonText: t('dialog_confirm'),
        bgClose: false,
      })
      if (!confirmed) return
    }
        ...
        ? (
            <View style={styles.configuration}>
              ...
              <View style={styles.buttons}>
                <Button disabled={action != null} onPress={() => { void run('restore') }}>{t('setting_webdav_restore')}</Button>
